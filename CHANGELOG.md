...
- **`tokensave_affected` separates practical tests from broad dependency candidates.** Source files containing inline `#[test]` modules are reported as `inline_test_sources` instead of runnable test targets; candidates are classified as direct, same-crate integration, cross-crate consumer, or transitive low-confidence using dependency distance and Cargo package boundaries; and `recommended_tests` provides the focused high/medium-confidence suite while the backward-compatible `affected_tests` field retains all discovered test targets.
